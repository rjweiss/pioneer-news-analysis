...
pid_scored_topsites_byvisits_melt = melt(pid_scored_topsites_byvisits_melt, id.vars=c('domain','pid7'))
pid_scored_topsites_bytime_melt = melt(pid_scored_topsites_bytime_melt, id.vars=c('domain','pid7'))
...
labs(title='Density of daily average alignment score',
     subtitle='By self-reported party identification',
    color='') +
 xlab('Daily average alignment score (Bakshy, Messing, and Adamic (2015))') + ylab('') +
theme_bw() + theme(legend.position = 'bottom') +
guides(color=guide_legend(label.position='bottom',title.position='top')) +
  scale_color_fivethirtyeight(name='') +
...
visits_fita = glm(posttreatment ~ pretreatment + branch, data=visits)
#visits_fitb = glm(posttreatment ~ pretreatment*prestage_avg_score + branch, data=visits)
visits_fitc = glm(posttreatment ~ pretreatment + branch*prestage_avg_score, data=visits)

plot_model(visits_fita,rm.terms='pretreatment')
#plot_model(visits_fitc,rm.terms=c('pretreatment', 'prestage_avg_score'))
sjp.int(visits_fitc, show.ci=T)

out = stargazer(visits_fita, visits_fitc,
          title='Regression predicting total scored site visits. Base case is no intervention',
          no.space=T,
...
```{r}
# daily
# spec1_daily = scored_browsing %>% -->
#   inner_join(pid_tbl) %>% -->
#   filter(stage != 'treatment') %>% -->
#   filter(pid7 != 'O' & pid7 != 'N' & pid7 != 'I') %>% -->
#   group_by(pioneer_id, days_since_appearance, pid7) %>% -->
#   mutate(daily_avg_score =  mean(score, na.rm=T)) %>% -->
#   ungroup %>% -->
#   sdf_pivot(pioneer_id + branch + days_since_appearance + daily_avg_score + daily_active_time_s ~ stage) %>% -->
#   na.replace(0) %>% -->
#   mutate(pretreatment = pretreatment * log1p(daily_active_time_s), -->
#          posttreatment = posttreatment * log1p(daily_active_time_s)) %>% -->
#   arrange(pioneer_id, days_since_appearance) -->

# #library(lme4) -->
# fit1a_daily = glm(posttreatment ~ pretreatment + branch, data=spec1_daily) -->
# fit1b_daily = glm(posttreatment ~ pretreatment *log1p(daily_active_time_s)+ branch, data=spec1_daily) -->
# fit1c_daily = glm(posttreatment ~ pretreatment *daily_avg_score+ branch, data=spec1_daily) -->


# out = stargazer(fit1a_daily, fit1b_daily, fit1c_daily, -->
#                 intercept.bottom=F, -->
#                 title='General linear regression predicting daily post-treatment scored active time (in seconds). Base case is no intervention', -->
#                 no.space=T, -->
...
#plot(ggeffects::ggpredict(fit1c_daily, terms = c('pretreatment','daily_avg_score', 'branch')))
...
#   filter(pid7 != 'O' & pid7 != 'N' & pid7 != 'I') %>% -->
#   group_by(pioneer_id, stage) %>% -->
#   mutate(prestage_avg_score =  mean(score, na.rm=T), n=n()) %>% -->
#   ungroup %>%  -->
#   select(pioneer_id, prestage_avg_score, -stage) %>% -->
#   compute('pretreatment_avgscore_tbl') -->

spec1_stage = scored_browsing %>% 
  filter(stage!='treatment') %>%
  #inner_join(pid_tbl) %>%
  #filter(pid7 != 'O' & pid7 != 'N' & pid7 != 'I') %>%
  inner_join(pretreatment_avg_score) %>%
...
#  filter(stage != 'treatment') %>%
#  filter(pid7 != 'O' & pid7 != 'N' & pid7 != 'I') %>% 
#  group_by(pioneer_id, stage, pid7) %>%
#  mutate(stage_avg_score =  mean(score, na.rm=T)) %>%
#  ungroup %>%
#  sdf_pivot(pioneer_id + branch + days_since_appearance + stage_avg_score + daily_active_time_s ~ stage) %>%
#  na.replace(0) %>%
#  mutate(pretreatment = pretreatment * log1p(daily_active_time_s),
#         posttreatment = posttreatment * log1p(daily_active_time_s)) %>%
#  arrange(pioneer_id, days_since_appearance)

spec1_stage$branch = as.factor(spec1_stage$branch)
fit1a_stage= glm(posttreatment ~ pretreatment + branch, data=spec1_stage)
#fit1b_stage= glm(posttreatment ~ pretreatment*daily_active_time_s + branch, data=spec1_stage)
fit1c_stage= glm(posttreatment ~ pretreatment + branch*prestage_avg_score, data=spec1_stage)
plot_model(fit1a_stage, rm.terms='pretreatment')
sjp.int(fit1c_stage, show.ci=T)


out = stargazer(fit1a_stage, fit1c_stage,
                intercept.bottom=F,
...
#plot(ggeffects::ggpredict(fit1c_stage, terms = c('pretreatment','stage_avg_score', 'branch')))
```
...
# sdf_pivot(pioneer_id + branch + days_since_appearance + daily_avg_score + daily_active_time_s ~ stage) %>%
#  na.replace(0) %>%
#  mutate(pretreatment = pretreatment * daily_avg_score,
#        posttreatment = posttreatment * daily_avg_score) %>%
#  arrange(pioneer_id, days_since_appearance)

#fit2a_daily= glm(posttreatment ~ pretreatment + branch, data=spec2_daily)
#fit2b_daily= glm(posttreatment ~ pretreatment *log1p(daily_active_time_s) + branch, data=spec2_daily)
#fit2c_daily= glm(posttreatment ~ pretreatment *daily_avg_score + branch, data=spec2_daily)

#out = stargazer(fit2a_daily, fit2b_daily, fit2c_daily,
#                intercept.bottom=F,
#                title='Effect of doorhanger intervention on daily average post-treatment score. Base case is no intervention',
#                no.space=T,
...
# sdf_pivot(pioneer_id + branch + prestage_avg_score + daily_active_time_s ~ stage) %>%
# group_by(pioneer_id) %>%
# mutate(pretreatment = sum(pretreatment * log1p(daily_active_time_s), na.rm=T),
#        posttreatment = sum(posttreatment * log1p(daily_active_time_s), na.rm=T)) %>%
#  collect %>%
#  distinct(pioneer_id, branch, prestage_avg_score, posttreatment, pretreatment) %>%
#  na.omit

#XXX added abs value
spec2_stage = scored_browsing %>% 
  filter(stage != 'treatment') %>%
  #inner_join(pid_tbl) %>%
  #filter(pid7 != 'O' & pid7 != 'N' & pid7 != 'I') %>% 
  ...
  #distinct(pioneer_id, branch, posttreatment, pretreatment) %>%
  inner_join(pretreatment_avg_score) %>%
  na.omit %>%
  collect
#spec2_stage %>% filter(pioneer_id=='f0d2d9f2-5fbd-4803-9911-beaab103d7af')
...
#  mutate(stage_avg_score =  mean(score, na.rm=T)) %>%
#  ungroup %>%
#  sdf_pivot(pioneer_id + branch + stage_avg_score + daily_active_time_s ~ stage) %>%
#  na.replace(0) %>%
#  mutate(pretreatment = pretreatment * stage_avg_score,
#         posttreatment = posttreatment * stage_avg_score) 

spec2_stage$branch = as.factor(spec2_stage$branch)
fit2a_stage= glm(posttreatment ~ pretreatment + branch, data=spec2_stage)
#fit2b_stage= glm(posttreatment ~ pretreatment*log1p(daily_active_time_s) + branch*log1p(daily_active_time_s), data=spec2_stage)
#fit2b_stage= glm(posttreatment ~ pretreatment*log1p(daily_active_time_s) + branch*log1p(daily_active_time_s), data=spec2_stage)
fit2c_stage= glm(posttreatment ~ pretreatment + branch*prestage_avg_score, data=na.omit(spec2_stage))

plot_model(fit2a_stage, rm.terms='pretreatment')
sjp.int(fit2c_stage, show.ci=T)

out = stargazer(fit2a_stage, fit2c_stage,
                intercept.bottom=F,
