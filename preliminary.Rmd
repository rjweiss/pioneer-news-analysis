...
dwell_tbl = spark_read_parquet(sc, name='dwell_tbl', path=dwell_parquet_dir, repartition=500, memory=F) %>%
  select(-document_ids, log_events) #%>% compute('dwell_tbl')
  #sdf_register('dwell_tbl')

#scored_domains_tbl = sdf_broadcast(scored_domains)#, overwrite = T)
#scored_dwell_tbl = dwell_tbl %>% inner_join(scored_domains_tbl) %>% compute('scored_dwell_tbl')#%>% sdf_register('scored_dwell_tbl')
# ```
# 
...
#   filter(domain != 'youtube.com' & domain != 'google.com' & domain!= 'facebook.com') %>% # remove these sites as they are a huge amount of time
#   filter(visit_start_date > "2018-01-01" & visit_start_date <"2018-05-01") %>% # remove odd clients, likely clock skew
#   filter(days_since_appearance > 0 & days_since_appearance < 23) %>% # only look at study periods
#   mutate(stage = case_when(
#     days_since_appearance == 0 ~ 'enrollment',
...
#  x=reorder(score, score), fill=pid)) +
#  geom_density()
...
library(pander)
library(sjPlot)
fit1_xcontrol = glm(data=treatment_spark, formula=posttreatment~pretreatment + branch)
#pander(fit1)
summary(fit1_xcontrol)
#sjPlot::plot_model(fit1_xcontrol) #+ theme_bw()

fit1_xplacebo = glm(data=treatment_spark, formula=posttreatment~pretreatment + condition)
#sjPlot::plot_model(fit1_xplacebo) #+ theme_bw()
summary(fit1_xplacebo)

fit2 = glm(data=treatment_spark, formula=posttreatment~pretreatment*branch + branch)
#pander(fit2)
summary(fit2)
#sjPlot::plot_model(fit2)

fit2b = glm(data=treatment_spark, formula=posttreatment~pretreatment*condition + condition)
sjPlot::plot_model(fit2b)
summary(fit2b)


#install.packages("ggeffects")
library(ggeffects)
dat = ggeffects::ggpredict(fit2, terms = c('pretreatment','branch'))
plot(dat, facet=T) + theme_bw()

datb = ggeffects::ggpredict(fit2b, terms = c('pretreatment','condition'))
plot(datb, facet=T) + theme_bw()
...
fit1_xcontrol = glm(data=treatment_pid_spark, formula=posttreatment~pretreatment + branch + pid)
#pander(fit1)
sjPlot::plot_model(fit1_xcontrol) 
summary(fit1_xcontrol)

fit1_xplacebo = glm(data=treatment_pid_spark, formula=posttreatment~pretreatment + condition + pid)
sjPlot::plot_model(fit1_xplacebo) #+ theme_bw()
summary(fit1_xplacebo)

fit2 = glm(data=treatment_pid_spark, formula=posttreatment~pretreatment + branch* pid)
#pander(fit2)
sjPlot::plot_model(fit2)
summary(fit2)

fit2b = glm(data=treatment_pid_spark, formula=posttreatment~pretreatment + condition*pid)
sjPlot::plot_model(fit2b)
summary(fit2b)
...
#install.packages("ggeffects")
library(ggeffects)
dat = ggeffects::ggpredict(fit2, terms = c('pretreatment','pid', 'branch'))
plot(dat) + theme_bw()

datb = ggeffects::ggpredict(fit2b, terms = c('pretreatment','pid', 'condition'))
plot(datb) + theme_bw()
